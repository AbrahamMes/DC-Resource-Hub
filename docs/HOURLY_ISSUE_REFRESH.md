...
The backend refreshes ACC issues and Controller-category assets for every configured project in every site 30 seconds after startup and once every 60 minutes afterward. Because the deployment uses one backend instance, only one scheduler runs. A run is skipped if the previous run is still active.
